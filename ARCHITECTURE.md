...
- `profile` (the signed-in user, `GET /me`), `mail-folders` (`me.mailFolders`: list, get, create, rename, delete), `messages` (`me.messages`: list, get with `--raw`, create draft from MIME, update, send, copy, move, delete), `attachments` (`me.messages.attachments`: list, get/download, delete).
