...
The read path stops parsing filenames by hand and calls io-maildir instead. That is the layering the contributing guide asks for, deciding what a Maildir name means being storage semantics, and it means the format has one definition rather than a copy in this repo that can drift from it.
...
On io-maildir 0.2.1 the entry locate behind every flag store discards the slot letters before renaming, so a flag operation strips a keyword it never touched. That reaches further than `flag add` and `flag remove`: `message read --seen` marks seen through the same path, so reading a message would delete its keywords. The fix is pimalaya/io-maildir#3, merged upstream and awaiting the 0.2.2 tag, so this change requires `io-maildir = "0.2.2"` rather than shipping the caveat.
