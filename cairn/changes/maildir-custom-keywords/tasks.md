...
- [x] src/config.rs: add the `maildir.dovecot-keywords` and `maildir.keywords-header` fields, with a local `KeywordHeaderConfig` mirror so the config schema keeps compiling under every feature subset.
- [x] src/maildir/client.rs: convert the mirror into io-maildir's `KeywordHeader` and set both knobs on the inner client, covering the CLI and shared construction sites at once.
