    ...
    pub fn execute(
        ...
        if backend.allows_imap() {
            if let Some(imap_config) = account_config.imap.take() {
                use io_email::imap::flag_add::{FlagAdd, FlagAddResult};
                ...
                let mut coroutine =
                    FlagAdd::new(session.context, mailbox, sequence_set, imap_flags, true);
                ...
                loop {
                    match coroutine.resume(arg.take()) {
                        ...
                        FlagAddResult::WantsRead => {
                            let n = session.stream.read(&mut buf)?;
                            arg = Some(&buf[..n]);
                        }
                        FlagAddResult::WantsWrite(bytes) => {
        ...
        if backend.allows_jmap() {
            if let Some(jmap_config) = account_config.jmap.take() {
                use io_email::jmap::flag_add::{FlagAdd, FlagAddResult};
        ...
        if backend.allows_maildir() {
            if let Some(maildir_config) = account_config.maildir.take() {
                use io_email::maildir::flag_add::{FlagAdd, FlagAddArg, FlagAddResult};
                use io_maildir::{
                    flag::{Flag, Flags},
                    maildir::Maildir,
                };
                ...
                for id in &self.ids.inner {
                    let mut coroutine =
                        FlagAdd::new(maildir.clone(), id.as_str(), maildir_flags.clone());
                    let mut arg: Option<FlagAddArg> = None;
